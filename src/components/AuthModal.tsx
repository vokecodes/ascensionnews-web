...
export default function AuthModal({
  ...
  const handleForgotSubmit = async (values: { email: string }) => {
    try {
      ...
      setMode("reset");
      forms.reset.reset({ email: values.email, code: "" });
    } catch (err) {
      toast.error(getErrorMessage(err));
    }
  };

  const handleResetSubmit = async (values: ResetPasswordPayload) => {
    try {
    ...
    >
      ...
      >
        ...
        <form
          onSubmit={
            mode === "login"
              ? forms.login.handleSubmit(handleLoginSubmit)
              : mode === "signup"
              ? forms.signup.handleSubmit(handleSignupSubmit)
              : mode === "forgot"
              ? forms.forgot.handleSubmit(handleForgotSubmit)
              : forms.reset.handleSubmit(handleResetSubmit)
          }
        ...
        >
          ...
          >
            {forms[mode].formState.isSubmitting
              ? "Please wait..."
              : primaryCtaMap[mode]}
          </button>
        </form>
