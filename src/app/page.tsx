...
import { getHotNews, getTrendingNews, getNewsByTopic, getNewsByLocation } from '@/api/news';
...
export default function Home() {
  ...
  const [categories, setCategories] = useState<Topic[]>([{ key: 'home', label: 'Home', emoji: '🏡' }]);
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        ...
        setCategories([{ key: 'home', label: 'Home', emoji: '🏡' }, ...fetchedTopics]);
      } catch (error) {
  ...
  const fetchData = async (pageNum: number, isLoadMore: boolean = false) => {
    ...
    try {
      if (activeCategory === 'Home') {
        if (pageNum === 1) {
          const [hot, trending] = await Promise.all([getHotNews(1, 10), getTrendingNews(1, 10)]);
          setHotNews(hot);
        ...
        if (activeCategory === 'Trending') {
          ...
          if (userLocation) {
            news = await getNewsByLocation(userLocation.country_name, userLocation.region, userLocation.city, pageNum, 10);
          } else {
            setLoading(false);
          ...
          if (category) {
            news = await getNewsByTopic(category.key, pageNum, 10);
          }
        }
  ...
  useEffect(() => {
    // Initial fetch when category or necessary deps change
    if (activeCategory === 'Home' || categories.length > 1) {
      if (activeCategory === 'Local' && !userLocation) {
        return;
      }
      // Pass page 1 always for the effect-triggered fetch (reset)
      fetchData(1, false);
    }
  ...
  if (loading && !topicNews.length && !hotNews.length && activeCategory !== 'Local') {
    return (
  ...
  return (
    <div className="min-h-screen bg-[#202124]">
      {/* Header */}
      <Header activeCategory={activeCategory} setActiveCategory={setActiveCategory} categories={categories} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2 bg-[#292a2d] rounded-lg p-4">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-[#8ab4f8] text-lg font-medium">Top stories</h2>
                </div>
                {hotNews.length > 0 && (
              ...
              <div className="bg-[#292a2d] rounded-lg p-4">
                <h2 className="text-[#8ab4f8] text-sm font-medium mb-4">Picks for you</h2>
                <div className="space-y-4">
